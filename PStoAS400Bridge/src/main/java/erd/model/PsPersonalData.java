...
public class PsPersonalData implements Serializable {
	...
	public static PsPersonalData findByOriginalHireEmployeeId(String employeeId) {
		//SELECT FROM PS_PERSONAL_DATA P, PS_ORIG_HIR_EMP_VW P2                                                        
		//WHERE P.EMPLID = $PSEmplid AND P2.EMPLID = P.EMPLID                                                     
		EntityManagerFactory emfactory = Persistence.createEntityManagerFactory("PStoAS400Bridge");
		EntityManager em = emfactory.createEntityManager();
	    try {
	    	List<PsPersonalData> resultList = (List<PsPersonalData>) em.createQuery(
	    			"SELECT p FROM PsPersonalData p, PsEmployeeOriginalHire p2 "
	...
	public static PsPersonalData findByEmployeeId(String employeeId) {
		//SELECT P.LANG_CD FROM PS_PERSONAL_DATA P
		//WHERE P.EMPLID = $EmplId
		EntityManagerFactory emfactory = Persistence.createEntityManagerFactory("PStoAS400Bridge");
		EntityManager em = emfactory.createEntityManager();
	    try {
	    	List<PsPersonalData> resultList = (List<PsPersonalData>) em.createQuery(
	    			"SELECT p FROM PsPersonalData p "
	    			+ "WHERE UPPER(TRIM(p.employeeId)) = UPPER(TRIM(:employeeId)) "
	    			, PsPersonalData.class)
	    		    .setParameter("employeeId", employeeId)
	    		    .getResultList();
